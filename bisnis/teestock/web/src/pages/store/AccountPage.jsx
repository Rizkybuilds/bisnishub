    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12 space-y-8">
      ...
      <div className="p-6 sm:p-8 rounded-3xl bg-ts-surface/80 backdrop-blur-xl border border-white/[0.1] shadow-glass-card shadow-glass-inset flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
        <div className="flex items-center gap-4">
          <div className="relative w-16 h-16 rounded-2xl bg-gradient-to-br from-ts-terracotta to-[#9E4620] flex items-center justify-center font-extrabold text-white text-2xl shadow-glow-terracotta border border-white/20 shrink-0">
      ...
      {activeTab === 'orders' && (
        <div className="space-y-4 animate-in fade-in duration-300">
          {userOrders.length === 0 ? (
            <div className="p-12 text-center rounded-3xl bg-ts-surface/60 border border-white/[0.08] space-y-4">
              <Package className="w-12 h-12 text-ts-muted mx-auto" />
              <div className="space-y-1">
                <h3 className="text-base font-bold text-white">Belum Ada Riwayat Pesanan</h3>
            ...
            <div className="space-y-4">
              {userOrders.map((order) => {
                const statusMap = {
                  pending: { label: 'Menunggu Konfirmasi', color: 'bg-amber-500/15 text-amber-300 border-amber-500/30' },
                  dtf: { label: 'Cetak Film DTF HD', color: 'bg-blue-500/15 text-blue-300 border-blue-500/30' },
                  press: { label: 'Heat Press 155°C', color: 'bg-ts-terracotta/20 text-ts-terracotta border-ts-terracotta/40' },
                  pack: { label: 'Packing Polymailer Distro', color: 'bg-purple-500/15 text-purple-300 border-purple-500/30' },
                  shipped: { label: 'Sedang Dikirim Kurir', color: 'bg-ts-teal/20 text-teal-300 border-ts-teal/40' },
                  review: { label: 'Pesanan Selesai', color: 'bg-ts-green/15 text-green-300 border-ts-green/30' },
                };
                ...
                return (
                  <div
                    key={order.id}
                    className="p-5 sm:p-6 rounded-3xl bg-ts-surface/80 backdrop-blur-xl border border-white/[0.09] shadow-glass-card space-y-4"
                  >
                    ...
                    <div className="pt-2 flex items-center justify-between text-xs text-ts-muted">
                      ...
                      <a
                        ...
                        className="text-ts-kremMuted hover:text-white flex items-center gap-1"
                      >
      ...
      {activeTab === 'profile' && (
        <form onSubmit={handleSaveProfile} className="max-w-2xl p-6 sm:p-8 rounded-3xl bg-ts-surface/80 backdrop-blur-xl border border-white/[0.09] shadow-glass-card space-y-6 animate-in fade-in duration-300">
          <div className="border-b border-white/[0.08] pb-4">
            <h3 className="text-base font-bold text-white">Informasi Penerima &amp; Alamat Pengiriman</h3>
          ...
          {profileMsg && (
            <div className={`p-3.5 rounded-xl text-xs flex items-center gap-2 ${
              profileMsg.type === 'success' 
                ? 'bg-ts-green/15 border border-ts-green/30 text-green-300' 
          ...
          <div>
            <label className="block text-xs font-bold text-white mb-1.5">Alamat Lengkap</label>
            <textarea
      ...
      {activeTab === 'vouchers' && (
        <div className="space-y-5 animate-in fade-in duration-300">
          <div className="border-b border-white/[0.08] pb-3">
            <h3 className="text-base font-bold text-white">Voucher &amp; Promo Aktif Kamu</h3>
            <p className="text-xs text-ts-kremMuted mt-0.5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {vouchers.map((v) => (
              <div
                key={v.code}
                className="p-5 rounded-3xl bg-gradient-to-br from-ts-surface/90 to-[#1C1A18] border border-ts-terracotta/30 shadow-glass-card shadow-glass-inset flex flex-col justify-between space-y-4"
              >
                <div className="space-y-2">
                ...
                <div className="pt-3 border-t border-white/[0.06] flex items-center justify-between gap-2">
                  <span className="font-mono text-sm font-bold text-white px-2.5 py-1 rounded-lg bg-white/[0.06] border border-white/10">
                    {v.code}
                  </span>
                  <button
                    onClick={() => handleCopyVoucher(v.code)}
                    className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-ts-terracotta hover:bg-ts-terracotta/90 text-white text-xs font-bold transition-all"
                  >
                    {copiedCode === v.code ? (
                      <>
      ...
      {activeTab === 'partner' && (
        <div className="max-w-3xl p-6 sm:p-8 rounded-3xl bg-ts-surface/80 backdrop-blur-xl border border-white/[0.09] shadow-glass-card space-y-6 animate-in fade-in duration-300">
          <div className="flex items-center justify-between border-b border-white/[0.08] pb-4">
            <div>
              <h3 className="text-base font-bold text-white">Program Kemitraan Dropship &amp; Reseller</h3>
              <p className="text-xs text-ts-kremMuted mt-0.5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="p-4 rounded-2xl bg-white/[0.03] border border-white/[0.06] space-y-2">
              <span className="text-xs font-bold text-white block">Harga Khusus Mitra</span>
            ...
            <div className="p-4 rounded-2xl bg-white/[0.03] border border-white/[0.06] space-y-2">
              <span className="text-xs font-bold text-white block">Pengiriman White-Label</span>
              <p className="text-xs text-ts-kremMuted leading-relaxed">
          ...
          <div className="p-5 rounded-2xl bg-gradient-to-r from-ts-mustard/15 to-transparent border border-ts-mustard/30 flex flex-col sm:flex-row items-center justify-between gap-4">
            <div>
              <h4 className="text-sm font-bold text-white">Ajukan Upgrade ke Akun Mitra Dropship</h4>
              <p className="text-xs text-ts-kremMuted mt-0.5">
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="shrink-0 px-5 py-2.5 rounded-xl bg-ts-mustard hover:bg-ts-mustard/90 text-zinc-950 font-bold text-xs flex items-center gap-2 shadow-md transition-all"
            >
              <Users className="w-4 h-4" />
